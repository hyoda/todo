...
const local_data = JSON.parse(localStorage.getItem('todo') as string)
...
export default createStore({
  ...
  mutations: {
    ...
    DELETE_TODO(state, payload) {
      const index = state.todos.findIndex((todo: { id: any }) => todo.id === payload)
    ...
    CHANGE_TODO(state, payload) {
      const id = payload.id
      const index = state.todos.findIndex((todo: { id: any }) => todo.id === id)
